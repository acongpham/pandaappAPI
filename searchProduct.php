...
$query="select product.productId,product.name,product.price,product.discount,shop.shopName,product.idShop from Product inner join Shop on Product.idShop=Shop.idShop 
        ...
        INNER JOIN category on category.cateId=sub_category.cateId WHERE account.accountStatus=1 AND category.categoryName LIKE '%".$keysearch."%' OR shop.shopName LIKE '%".$keysearch."%'  OR product.name LIKE '%".$keysearch."%'";
