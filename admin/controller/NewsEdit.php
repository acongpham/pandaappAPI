...
if(isset($_POST['uploadnews']))
{
    if (isset($_FILES['avatar']))
    {
        ...
        if ($_FILES['avatar']['error'] > 0)
        {
            ...
            {
                unlink($row2['imageNews']);
            }
            $titleNews = $_POST['titleNews'];
            $detailNews = $_POST['detailNews'];
            $sqlinsert = "UPDATE `news` SET `newstitle`='$titleNews',`detailNews`='$detailNews',`imageNews`='$file_path',`dateCreated`= now()  WHERE newsId=$newsId";
                    ...
                    <div class="panel-body">
                        <form action="newsinsert.php" enctype="multipart/form-data" method="post">
                            <div class="form-group">
                                <label>Id tin tức:</label>
                                <input type="text" class="form-control" name="titleNews" value="<?php echo $_GET['newsId']?>">
                            </div>

                            <div class="form-group">
                                <label>Tiêu đề tin tức:</label>
                                <input type="text" class="form-control" name="titleNews" value="<?php echo $titleNews?>">
                            </div>
                            <div class="form-group">
